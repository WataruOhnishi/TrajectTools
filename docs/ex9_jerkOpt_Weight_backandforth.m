...
n = 24; % number of knots, typical 8
...
n_sample = 1000; % number of sampled point
t_sample = linspace(0,tfin,n_sample).';
t_weight = exp(t_sample); % t_sample.^2
obj = norm(jrk.f(t_sample).*t_weight,2);
...
h = plot(t_sample,max(y4)*t_weight/max(t_weight),'m-.');
legend(h,'weighting')
xlabel('time [s]');
title('jerk');
